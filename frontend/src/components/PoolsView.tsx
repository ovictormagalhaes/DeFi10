...
function sumUnclaimed(rewards: Token[]): number {
  ...
  return rewards.reduce((s, r) => {
    const v = (r as any).pending ?? (r as any).unclaimed ?? (r as any).rewardAmount ?? (r as any).accrued ?? (r as any).amount ?? (r as any).balance;
    const n = parseFloat(v);
...
function getUserValue(raw: any): number {
  ...
  if (toks.length) {
    return toks.reduce((s, t) => s + safeNum((t as any).totalPrice ?? (t as any).value ?? (t as any).financials?.totalPrice), 0);
  }
...
const PoolsView: React.FC<PoolsViewProps> = ({ getLiquidityPoolsData }) => {
  ...
  const enriched: EnrichedPool[] = React.useMemo(() => pools.map((p, i) => {
    ...
    const rewardsTotalValue = uncollectedFeeTokens.reduce((s: number, t: any) => s + (parseFloat(
