...
interface AssetTypeOptionEntry {
  value: number | string;
  label: string;
}

interface RebalanceItemDialogProps {
  open: boolean;
  editing?: boolean;
  title?: string;
  description?: string;
  assetType: number | string;
  setAssetType: (value: number | string) => void;
  assetId: string;
  setAssetId: (value: string) => void;
  assetIds: AssetIdEntry[];
  setAssetIds: (value: AssetIdEntry[]) => void;
  ...
  ASSET_TYPE_OPTIONS: AssetTypeOptionEntry[];
  RebalanceReferenceType: Record<string, string>;
  ITEM_TYPES: Record<string, number>;
  AssetDropdown: React.ComponentType<Record<string, unknown>>;
  TokenDisplay: React.ComponentType<Record<string, unknown>> | null;
  tokensList: SelectOption[];
  protocolsList: SelectOption[];
  getOptionsForType?: (type: number | string) => SelectOption[];
  canAdd: boolean;
        ...
        <div className="rebalance-form-grid" style={{ marginTop: 4 }}>
          ...
          <div className="form-group">
            ...
            {assetIds.length > 0 && (
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {assetIds.map((asset: AssetIdEntry, idx: number) => {
                  ...
                  const raw: Record<string, any> = (option?.raw as Record<string, any>) || {};
          ...
          <div className="form-group">
            ...
            >
              ...
              {Object.values(RebalanceReferenceType).map((rt: string) => (
          ...
          <div className="form-group">
            ...
            {referenceType === RebalanceReferenceType.TotalWallet ? (
              ...
              >
                ...
                {referenceOptions.map((opt: SelectOption, idx: number) => (
