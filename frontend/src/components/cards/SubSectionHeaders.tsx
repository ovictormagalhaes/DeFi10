 ...
 */
const SubSectionItemWithDropdown: React.FC<SubSectionItemWithDropdownProps> = ({
  label,
  values,
  defaultType = 'apr',
  defaultPeriod = 'day',
  ...
  const typeOptions = hasTypes
    ? Object.keys(values).map((type) => {
        ...
        switch (lowerType) {
          ...
          default:
            return { key: type, label: type.charAt(0).toUpperCase() + type.slice(1) };
        }
      })
    : [{ key: 'apr', label: 'APR' }];

  // Get current value based on selected type and period
  let currentValue: unknown, currentColor: string | undefined;
  if (isSimpleValue) {
 ...
 */
interface LendingSubSectionHeaderProps {
  data: WalletItem[];
...
export const LendingSubSectionHeader: React.FC<LendingSubSectionHeaderProps> = ({
  ...
  let avgHealthFactor: number | null = null;
 ...
 */
interface LiquiditySubSectionHeaderProps {
  data: WalletItem[];
}
