 ...
 */
const ProtocolGroupCard: React.FC<ProtocolGroupCardProps> = ({
  protocolName,
  chainName,
  positions = [],
  healthFactor = null,
  ...
  const protocol = (firstPosition.protocol || positions[0]?.protocol || {}) as {
    name?: string;
    logo?: string;
    icon?: string;
    iconUrl?: string;
  ...
  positions.forEach((item) => {
    ...
    const itemApy =
      (item.additionalData?.projections as any[])?.find((p: any) => p.type === 'apy')?.metadata
        ?.value ||
      item.additionalInfo?.apy ||
      0;
    const supplyRate =
      position.supplyRate || position.apy || (supplyValue > borrowValue ? itemApy : 0);
    const borrowRate =
      position.borrowRate ||
      position.borrowApy ||
      (borrowValue > supplyValue ? Math.abs(itemApy) : 0);
    ...
    // Collect projections
    const projections =
      item.additionalData?.projections || item.additionalInfo?.projections || position?.projections;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onMouseEnter={(e) =>
          (e.currentTarget.style.backgroundColor = theme.bgHover || theme.bgPanel)
        }
            ...
            {[...visiblePositions]
              ...
              .flatMap((item, posIndex) => {
                ...
                return uniqueTokens.map((token, tokenIndex) => {
                  ...
                  const isBorrowToken =
                    tokenType.includes('borrowed') || tokenType.includes('borrow');
