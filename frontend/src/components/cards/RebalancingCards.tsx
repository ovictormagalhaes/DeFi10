...
interface RebalancingCardsProps {
  entries: StrategyEntry[];
  onEdit: (entry: StrategyEntry) => void;
  onDelete: (id: string) => void;
  entryCurrentValues: Map<string, number>;
  bucketCurrentSums: Map<string, number>;
  bucketNoteSums: Map<string, number>;
  bucketKey: (entry: StrategyEntry) => string;
  tokensList: AssetOption[];
  poolsList: AssetOption[];
  lendingList: AssetOption[];
  stakingList: AssetOption[];
}

const RebalancingCards: React.FC<RebalancingCardsProps> = ({
  entries = [],
  onEdit,
  onDelete,
  entryCurrentValues,
  ...
  const toggleAssets = (rowId: string) => {
  ...
  const fmtPct = (n: number) => formatPercent(n, { decimals: 2 });
  const fmtUSD = (n: number) => maskValue(formatUsd(n, { decimals: 2 }));
        ...
        return (
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {(() => {
                ...
                const renderAsset = (asset, assetIdx) => {
                  ...
                  const assetType =
                    typeof asset.type === 'number' ? asset.type : asset.type || row.assetType;
                  ...
                  if (assetType === RebalanceAssetType.Wallet) {
                    ...
                    if (Array.isArray(pos?.tokens) && pos.tokens.length) {
                      ...
                      const t1 =
                        pos?.token1 || pos?.tokenB || pos?.quoteToken || pos?.secondaryToken;
                ...
                return (
                  <>
                    ...
                    {hasMultiple &&
                      isExpanded &&
                      remainingAssets.map((asset, idx) => renderAsset(asset, idx + 1))}
            ...
            >
              ...
              >
                {diffPct > 0 ? '▲' : diffPct < 0 ? '▼' : '•'}{' '}
                {formatPercent(diffPct, { decimals: 2, sign: true })}
              </span>
              <span
                style={{
                  fontSize: 11,
