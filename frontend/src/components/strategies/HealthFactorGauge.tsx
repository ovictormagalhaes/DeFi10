...
export const HealthFactorGauge: React.FC<HealthFactorGaugeProps> = ({
  ...
  const warningZone = describeArc(
    startAngle - totalAngle * normalizedCritical,
    startAngle - totalAngle * normalizedWarning
  );
  const cautionZone = describeArc(
    startAngle - totalAngle * normalizedWarning,
    startAngle - totalAngle * normalizedTarget
  );
    ...
    >
      <svg width={size} height={size * 0.7} style={{ overflow: 'visible' }}>
        ...
        <line
          ...
          strokeLinecap="round"
        />
        <circle cx={centerX} cy={centerY} r={6} fill={statusColor} />
      </svg>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div
            style={{ width: '12px', height: '3px', background: '#dc2626', borderRadius: '2px' }}
          />
          <span>Danger</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div
            style={{ width: '12px', height: '3px', background: '#f97316', borderRadius: '2px' }}
          />
          <span>Critical</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div
            style={{ width: '12px', height: '3px', background: '#fbbf24', borderRadius: '2px' }}
          />
          <span>Warning</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div
            style={{ width: '12px', height: '3px', background: theme.success, borderRadius: '2px' }}
          />
          <span>Safe</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <div
            style={{ width: '8px', height: '8px', background: theme.accent, borderRadius: '50%' }}
          />
          <span>Target</span>
        </div>
      </div>
