...
import {
  WalletItemType,
  WalletItemTypeLabels,
  WalletItemTypeColors,
} from '../../../constants/walletItemTypes';
...
export const useChartData = ({
  ...
  const portfolioData = useMemo(() => {
    const walletValue = walletTokens.reduce((sum: number, tokenData) => {
      ...
      const price =
        (token as Record<string, unknown>).totalPrice ??
        (token as Record<string, unknown>).financials;
      return (
        sum +
        (parseFloat(
          String((token as TokenLike).financials?.totalPrice ?? (token as TokenLike).totalPrice)
        ) || 0)
      );
    }, 0);

    const liquidityValue = groupDefiByProtocol(liquidityData).reduce((total: number, group) => {
      return (
        total +
        group.positions.reduce(
          (sum: number, pos) =>
            sum +
            (pos.tokens?.reduce((tokenSum: number, token) => {
              const price =
                parseFloat(String(token.financials?.totalPrice ?? token.totalPrice)) || 0;
    ...
    const lendingValue = groupDefiByProtocol(lendingData).reduce((grand: number, group) => {
      const groupSum = group.positions.reduce((sum: number, pos) => {
        const tokens = Array.isArray(pos.tokens)
          ...
          : [];
        const net = tokens.reduce(
          (s: number, t) => s + signedTokenValue(t, pos as Record<string, unknown>),
          0
        );
        return sum + net;
      }, 0);
    ...
    const stakingValue = stakingData.reduce((total: number, position) => {
      const balance =
        parseFloat(
          String(position.additionalData?.balance ?? (position as Record<string, unknown>).balance)
        ) || 0;
    ...
    const lockingValue = lockingGroups.reduce((total: number, group) => {
      const groupValue = group.positions.reduce((sum: number, pos) => {
        const tokensValue =
          pos.tokens?.reduce((tokenSum: number, token) => {
    ...
    return {
      ...
      composition: [
        {
          name: WalletItemTypeLabels[WalletItemType.WALLET],
          value: walletValue,
          color: WalletItemTypeColors[WalletItemType.WALLET],
          category: 'wallet',
        },
        {
          name: WalletItemTypeLabels[WalletItemType.LIQUIDITY_POOL],
          value: liquidityValue,
          color: WalletItemTypeColors[WalletItemType.LIQUIDITY_POOL],
          category: 'liquidity',
        },
        {
          name: WalletItemTypeLabels[WalletItemType.LENDING_AND_BORROWING],
          value: lendingValue,
          color: WalletItemTypeColors[WalletItemType.LENDING_AND_BORROWING],
          category: 'lending',
        },
        {
          name: WalletItemTypeLabels[WalletItemType.LOCKING],
          value: lockingValue,
          color: WalletItemTypeColors[WalletItemType.LOCKING],
          category: 'locking',
        },
        {
          name: WalletItemTypeLabels[WalletItemType.STAKING],
          value: stakingValue,
          color: WalletItemTypeColors[WalletItemType.STAKING],
          category: 'staking',
        },
  ...
  const topTokens = useMemo(() => {
    const tokenMap = new Map<
      string,
      { symbol: string; name: string; value: number; logo?: string }
    >();
        ...
        if (pos.tokens && Array.isArray(pos.tokens)) {
          pos.tokens.forEach((token) => {
            ...
            addToken(
              ...
              token.name || '',
              value,
              token['logo' as string] as string | undefined
            );
          });
        }
      });
  ...
  const protocolDistribution = useMemo(() => {
    const protocolMap = new Map<
      string,
      { name: string; value: number; logo?: string; color: string; positionsCount: number }
    >();
      ...
      if (protocolValue > 0) {
        if (protocolMap.has(normalizedName)) {
          ...
          protocolMap.set(normalizedName, {
            ...
            value: protocolValue,
            logo:
              ((group.protocol as Record<string, unknown>).logoURI as string) ||
              ((group.protocol as Record<string, unknown>).logo as string),
    ...
    stakingData.forEach((position) => {
      ...
      if (protocolMap.has(protocolName)) {
        ...
        protocolMap.set(protocolName, {
          ...
          value: value,
          logo:
            ((position.protocol as Record<string, unknown>)?.logoURI as string) ||
            ((position.protocol as Record<string, unknown>)?.logo as string),
          color: colors[protocolMap.size % colors.length],
          positionsCount: 1,
        });
      }
  ...
  const chainDistribution = useMemo(() => {
    ...
    liquidityData.forEach((item) => {
      ...
      const value = (Array.isArray(tokens) ? tokens : []).reduce(
        (sum: number, t: TokenLike) =>
          sum + (parseFloat(String(t.financials?.totalPrice ?? t.totalPrice)) || 0),
        0
      );
    ...
    lendingData.forEach((item) => {
      ...
      const tokens = Array.isArray((pos as Record<string, unknown>).tokens)
        ? filterLendingDefiTokens(
            (pos as Record<string, unknown>).tokens as TokenLike[],
            showLendingDefiTokens
          )
        : [];
      const value = tokens.reduce(
        (sum: number, t) => sum + signedTokenValue(t, item as Record<string, unknown>),
        0
      );
      addToChain(chain as string, value);
    });
  ...
  const projectionData = useMemo(() => {
    ...
    lendingData.forEach((item) => {
      ...
      const additionalData = (item.additionalData || posRecord.additionalData || {}) as Record<
        string,
        unknown
      >;
      const additionalInfo = ((item as Record<string, unknown>).additionalInfo ||
        posRecord.additionalInfo ||
        {}) as Record<string, unknown>;
      const projections = (additionalData.projections ||
        additionalInfo.projections ||
        posRecord.projections ||
      ...
      const tokens = Array.isArray(posRecord.tokens)
        ...
        : [];
      const principalValue = tokens.reduce(
        (sum: number, t) => sum + signedTokenValue(t, item as Record<string, unknown>),
        0
      );
      ...
      projections.forEach((proj) => {
        ...
        const rate =
          ((proj.metadata as Record<string, unknown>)?.value as number) ||
          (posRecord.apy as number) ||
          (posRecord.supplyRate as number) ||
          0;
        ...
        existing.oneDay =
          (existing.oneDay as number) + (parseFloat(String(projection.oneDay)) || 0);
        existing.oneWeek =
          (existing.oneWeek as number) + (parseFloat(String(projection.oneWeek)) || 0);
        existing.oneMonth =
          (existing.oneMonth as number) + (parseFloat(String(projection.oneMonth)) || 0);
        existing.oneYear =
          (existing.oneYear as number) + (parseFloat(String(projection.oneYear)) || 0);

        if (rate != null && !isNaN(rate) && principalValue !== 0) {
          existing.totalRateWeighted =
            (existing.totalRateWeighted as number) + rate * Math.abs(principalValue);
    ...
    liquidityData.forEach((item) => {
      ...
      const additionalData = (posRecord.additionalData || item.additionalData || {}) as Record<
        string,
        unknown
      >;
      const additionalInfo = (posRecord.additionalInfo ||
        (item as Record<string, unknown>).additionalInfo ||
        {}) as Record<string, unknown>;
      const projections = (additionalData.projections ||
        additionalInfo.projections ||
        posRecord.projections ||
      ...
      tokens.forEach((token) => {
        const tokenValue = parseFloat(
          String(token.totalPrice || token.financials?.totalPrice || token.balanceFormatted || 0)
        );
      ...
      projections.forEach((proj) => {
        ...
        if (rate != null && !isNaN(rate) && principalValue > 0) {
          existing.totalRateWeighted =
            (existing.totalRateWeighted as number) + rate * principalValue;
