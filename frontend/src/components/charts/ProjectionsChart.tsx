...
export const ProjectionsChart: React.FC<Props> = ({
  ...
  const [selectedLendingType, setSelectedLendingType] = useState(lendingTypes[0]?.value || 'apy');
  ...
  const liquidityProjection = useMemo(
    () =>
      projections.liquidity.find((p) => p.type === selectedLiquidityType) ||
      projections.liquidity[0],
    ...
    return (
      ...
      >
        ...
        <div style={{ paddingTop: 6, borderTop: '1px solid rgba(255, 255, 255, 0.1)' }}>
          <span
            style={{ color: (chartColors as any).positive || chartColors.wallet, fontSize: 12 }}
          >
            Portfolio:{' '}
          </span>
  ...
  if (
    (projections.lending.length === 0 && projections.liquidity.length === 0) ||
    chartData.length === 0
  ) {
    return (
      <ChartContainer title="Projections Timeline" subtitle="Projected earnings over time">
        <div
          style={{
            height: 300,
            display: 'flex',
      ...
      >
        <div
          style={{
            display: 'grid',
            gridTemplateColumns:
              'minmax(100px, 120px) minmax(100px, 210px) repeat(4, minmax(80px, 1fr))',
            gap: 12,
            marginBottom: 8,
            minWidth: 500,
          }}
