      ...
      const rate =
        fees24h != null && totalVal > 0
          ...
          : apr != null
            ...
            : 0;
      const pairLabel =
        supplied.map((t: any) => t.symbol ?? '?').join(' / ') ||
...
const PoolCard: React.FC<{ item: any; onProjection: (t: any) => void }> = ({
  item,
  onProjection,
}) => {
  ...
  const displayRange =
    range && flipped && range.current && range.lower && range.upper
      ? {
          ...range,
  ...
  const inRange = displayRange
    ? (displayRange.inRange ??
      (displayRange.current >= displayRange.lower && displayRange.current <= displayRange.upper))
    : null;
  ...
  const pairLabel =
    supplied.map((t: any) => t.symbol ?? '?').join(' / ') || (item.position?.name ?? 'Position');
  ...
  const tierLabel =
    tierPercent != null && isFinite(tierPercent) ? `${(tierPercent * 100).toFixed(2)}%` : null;
      ...
      onClick={() =>
        onProjection({
          ...
          shareData: {
            ...
            token0Logo:
              supplied[0]?.logo ??
              supplied[0]?.thumbnail ??
              supplied[0]?.financials?.logo ??
              undefined,
            token1Logo:
              supplied[1]?.logo ??
              supplied[1]?.thumbnail ??
              supplied[1]?.financials?.logo ??
              undefined,
            ...
            totalFees:
              uncollectedVal + collectedVal > 0 ? uncollectedVal + collectedVal : undefined,
            rangeData:
    ...
    >
      ...
      <div className={s.feesSection}>
        <div className={s.feeRow}>
          ...
          <span
            className={s.feeVal}
            style={{ color: uncollectedVal > 0 ? 'var(--v2-green)' : undefined }}
          >
            <MaskedValue
              value={`$${uncollectedVal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            />
          </span>
        </div>
        <div className={s.feeRow}>
          ...
          <span className={s.feeVal}>
            <MaskedValue
              value={`$${collectedVal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            />
          </span>
        </div>
      </div>
      ...
      <div className={s.metrics}>
        <div className={s.metric}>
          ...
          <span
            ...
            style={{ color: apr != null && parseFloat(apr) > 0 ? 'var(--v2-green)' : undefined }}
          >
        ...
        <div className={s.metric}>
          ...
          <span
            className={s.metricValue}
            style={{
              color: inRange == null ? undefined : inRange ? 'var(--v2-green)' : 'var(--v2-red)',
            }}
          >
        ...
        <div className={s.metric}>
          ...
          <span className={s.metricValue}>{ageSeconds != null ? formatAge(ageSeconds) : '—'}</span>
        </div>
      </div>
    </div>
  );
