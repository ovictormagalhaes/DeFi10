...
function triggersEqual(a?: PurchaseTrigger, b?: PurchaseTrigger): boolean {
  ...
  if (a.type === 'window' && b.type === 'window')
    return a.window === b.window && a.direction === b.direction;
  return false;
}
...
function sortTriggers(triggers: PurchaseTrigger[]): PurchaseTrigger[] {
  return [...triggers].sort((a, b) => {
    if (a.type === 'window' && b.type === 'window')
      return (WINDOW_ORDER[a.window] ?? 999) - (WINDOW_ORDER[b.window] ?? 999);
...
function triggerLabel(t: PurchaseTrigger): string {
  if (t.type === 'price')
    return t.direction === 'below' ? `below $${t.target}` : `above $${t.target}`;
...
export const BestPurchaseWindowCard: React.FC<Props> = ({ strategy, onEdit, onDelete }) => {
  ...
  function getResult(
    ...
    idx: number
  ): PurchaseWindowEvalResult | undefined {
    if (!results) return undefined;
    const triggers = entryTriggers(entry);
    const match = results.find((r) => {
      if (r.assetKey !== entry.assetKey) return false;
      const rt = r.evaluations?.length
        ? r.evaluations.map((e) => e.trigger)
        : r.trigger
          ? [r.trigger]
          : [];
    ...
    <div className={s.card}>
      <div className={s.header} onClick={() => setCollapsed((v) => !v)}>
        ...
        <div className={s.headerMeta}>
          ...
          <div className={s.sub}>
            {strategy.purchaseWindowEntries.length} asset
            {strategy.purchaseWindowEntries.length !== 1 ? 's' : ''}
          </div>
        </div>
              ...
              <tbody>
                {strategy.purchaseWindowEntries.map((entry, ei) => {
                  ...
                  if (triggers.length <= 1) {
                    const t = triggers[0];
                    const evalForT = t
                      ? result?.evaluations?.find((e) => triggersEqual(e.trigger, t))
                      : undefined;
                    const triggerSignal = evalForT?.signal ?? false;
                    const refLabel = t
                      ? (() => {
                          if (t.type === 'price')
                            return result ? fmtPrice(result.currentPriceUsd) : '—';
                          const wmap: Record<string, number | undefined> = {
                            h1: md?.priceChangePercentage1h,
                            h24: md?.priceChangePercentage24h,
                            d7: md?.priceChangePercentage7d,
                            d30: md?.priceChangePercentage30d,
                            d200: md?.priceChangePercentage200d,
                            y1: md?.priceChangePercentage1y,
                            ytd: md?.priceChangePercentageYtd,
                          };
                    ...
                    return (
                      <tr key={ei}>
                        <td className={s.tl}>
                          <div className={s.cellWithLogo}>
                            {entry.token?.logo ? (
                              ...
                              <div className={s.logoFallback}>{entry.symbol.slice(0, 2)}</div>
                            )}
                        ...
                        <td
                          ...
                          style={{
                            fontSize: 11,
                            color: triggerSignal ? 'var(--v2-green)' : 'var(--v2-muted)',
                          }}
                        >
                          {t ? triggerLabel(t) : '—'}
                        </td>
                        ...
                        <td className={s.tc}>
                          <span className={s.pwSignal}>
                            ...
                            <span style={{ color: signal ? 'var(--v2-green)' : 'var(--v2-dim)' }}>
                              {result ? (signal ? 'Active' : 'Waiting') : '—'}
                            </span>
                          </span>
                        </td>
                      </tr>
                      ...
                      <tr key={`${ei}-${ti}`}>
                        {ti === 0 && (
                          <>
                            <td
                              className={s.tl}
                              rowSpan={triggers.length}
                              style={{
                                borderRight: '1px solid var(--v2-border-sub)',
                                verticalAlign: 'top',
                                paddingTop: 10,
                              }}
                            >
                            ...
                            >
                              {result ? fmtPrice(result.currentPriceUsd) : '—'}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage1h)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage24h)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage7d)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage30d)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage200d)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentage1y)}
                            </td>
                            <td
                              className={s.tr}
                              rowSpan={triggers.length}
                              style={{ verticalAlign: 'top', paddingTop: 10 }}
                            >
                              {pctCell(md?.priceChangePercentageYtd)}
                            </td>
                          </>
                        )}
                        <td
                        ...
                        {ti === 0 && (
                          <td
                            ...
                            style={{
                              ...
                              borderLeft: '1px solid var(--v2-border-sub)',
                            }}
                          >
                            <span className={s.pwSignal}>
                              <span className={`${s.pwDot} ${signal ? s.pwDotOn : s.pwDotOff}`} />
