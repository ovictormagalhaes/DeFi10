...
export const HealthFactorCard: React.FC<Props> = ({ statuses, config, onEdit, onDelete }) => {
  ...
  const dotColor = criticalCount > 0 ? 'var(--v2-red)' : warnCount > 0 ? 'var(--v2-yellow)' : 'var(--v2-green)';
  const statColor = criticalCount > 0 ? 'var(--v2-red)' : warnCount > 0 ? 'var(--v2-yellow)' : 'var(--v2-green)';
  const statText = criticalCount > 0 ? `${criticalCount} critical` : warnCount > 0 ? `${warnCount} warning` : 'All safe';
  ...
  const hfBarColor = (st: HealthFactorStatus['status']) =>
    st === 'danger' || st === 'critical' ? 'var(--v2-red)' : st === 'warning' ? 'var(--v2-yellow)' : 'var(--v2-green)';

  const chipClass = (st: HealthFactorStatus['status']) =>
    st === 'danger' || st === 'critical' ? s.chipDanger : st === 'warning' ? s.chipWarn : s.chipSafe;

  const chipLabel = (st: HealthFactorStatus['status']) =>
    st === 'danger' ? 'Danger' : st === 'critical' ? 'Critical' : st === 'warning' ? 'Warning' : 'Safe';
    ...
    <div className={s.card}>
      <div className={s.header} onClick={() => setCollapsed(v => !v)}>
        ...
        <div className={s.headerMeta}>
          ...
          <div className={s.sub}>{statuses.length} position{statuses.length !== 1 ? 's' : ''}</div>
        </div>
      ...
      {!collapsed && statuses.length > 0 && (
        <div className={s.body}>
                    ...
                    <tr key={i}>
                      <td className={s.tl}>
                        <div className={s.cellWithLogo}>
                          {st.protocolLogo && <img src={st.protocolLogo} alt={st.protocolName} className={s.tokenImg} onError={e => (e.target as HTMLImageElement).style.display = 'none'} />}
                          <span className={s.bold}>{st.protocolName}</span>
                        </div>
                      </td>
                      <td className={s.tl}>
                        <div className={s.cellWithLogo}>
                          {st.chainLogo && <img src={st.chainLogo} alt={st.chain} className={s.tokenImg} onError={e => (e.target as HTMLImageElement).style.display = 'none'} />}
                          <span>{st.chain ? st.chain.charAt(0).toUpperCase() + st.chain.slice(1) : ''}</span>
                        </div>
                      </td>
                      <td className={s.tc}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                          <div className={s.hfBarTrack}>
                            <div className={s.hfBarFill} style={{ width: `${barPct}%`, background: hfBarColor(st.status) }} />
                            <div className={s.hfMarker} style={{ left: `${criticalPct}%`, background: 'var(--v2-red)', opacity: 0.8 }} />
                            <div className={s.hfMarker} style={{ left: `${targetPct}%`, background: 'var(--v2-yellow)', opacity: 0.8 }} />
                          </div>
                          <span className={`${s.hfVal} ${hfColor(st.status)}`}>{st.current.toFixed(2)}</span>
                        </div>
                      </td>
                      <td className={s.tc} style={{ color: 'var(--v2-muted)' }}>{st.target.toFixed(2)}</td>
                      <td className={s.tc} style={{ color: 'var(--v2-red)' }}>{(st.criticalThreshold || 1.5).toFixed(2)}</td>
                      <td className={s.tc}>
                      ...
                      <td className={s.tc}>
                        <span className={`${s.chip} ${chipClass(st.status)}`}>{chipLabel(st.status)}</span>
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className={s.legend}>
            <span><span className={s.legendDot} style={{ background: 'var(--v2-yellow)' }} />Target HF (per position)</span>
            <span><span className={s.legendDot} style={{ background: 'var(--v2-red)' }} />Critical threshold (per position)</span>
          </div>
        </div>
      )}
    </div>
