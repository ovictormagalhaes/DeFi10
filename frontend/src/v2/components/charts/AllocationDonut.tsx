...
export const AllocationDonut: React.FC<Props> = ({
  walletTokens, lendingItems, poolItems, stakingItems, breakdown, initialView = 'portfolio',
}) => {
  ...
  const protocolSegments = useMemo<DonutSegment[]>(() => {
    ...
    const add = (proto: string, v: number) => { if (v > 0) map.set(proto, (map.get(proto) ?? 0) + v); };
  ...
  const tokenSegments = useMemo<DonutSegment[]>(() => {
    const map = new Map<string, number>();
    const add = (sym: string, v: number) => { if (v > 0) map.set(sym, (map.get(sym) ?? 0) + v); };
  ...
  const config = useMemo(() => ({
    ...
    protocol: { segments: protocolSegments, total: protocolSegments.reduce((sum, x) => sum + x.value, 0), center: 'Protocols' },
    token: { segments: tokenSegments, total: tokenSegments.reduce((sum, x) => sum + x.value, 0), center: 'Tokens' },
