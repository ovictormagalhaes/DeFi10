...
export const Gauge: React.FC<Props> = ({
  ...
  const zone =
    v < thresholds.safe ? 'var(--v2-green)' : v < thresholds.warn ? '#f59e0b' : 'var(--v2-red)';

  return (
    <div className={s.gaugeWrap}>
      <svg width={w} height={svgH} viewBox={`0 0 ${w} ${svgH}`}>
        <path
          d={arcPath(cx, cy, r, 0, safeEnd)}
          stroke="var(--v2-green)"
          strokeWidth={stroke}
          fill="none"
          strokeLinecap="round"
        />
        <path
          d={arcPath(cx, cy, r, safeEnd, warnEnd)}
          stroke="#f59e0b"
          strokeWidth={stroke}
          fill="none"
        />
        <path
          d={arcPath(cx, cy, r, warnEnd, 180)}
          stroke="var(--v2-red)"
          strokeWidth={stroke}
          fill="none"
          strokeLinecap="round"
        ...
        <text
          x={polar(cx, cy, r + 14, 0).x}
          y={polar(cx, cy, r + 14, 0).y + 4}
          textAnchor="middle"
          fontSize={10}
          fill="var(--v2-dim)"
        >
        ...
        <text
          x={polar(cx, cy, r + 14, 90).x}
          y={polar(cx, cy, r + 14, 90).y - 2}
          textAnchor="middle"
          fontSize={10}
          fill="var(--v2-dim)"
        >
        ...
        <text
          x={polar(cx, cy, r + 14, 180).x}
          y={polar(cx, cy, r + 14, 180).y + 4}
          textAnchor="middle"
          fontSize={10}
          fill="var(--v2-dim)"
        >
      ...
      <div className={s.gaugeBands}>
        ...
        <span className={s.bandWarn}>
          Warn {thresholds.safe}–{thresholds.warn}%
        </span>
