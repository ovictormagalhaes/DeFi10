...
function App(): JSX.Element {
  ...
  const toggleMaskValues = (): void => setMaskValues((m) => !m);
  ...
  const hasPulsedRef = useRef<boolean>(false);
  const [showPulse, setShowPulse] = useState<boolean>(false);
  const [showAccountHover, setShowAccountHover] = useState<boolean>(false);

  const [isWalletGroupModalOpen, setIsWalletGroupModalOpen] = useState<boolean>(false);
  ...
  const fetchProtocolsStatus = async (): Promise<void> => {
    ...
    try {
      ...
      setStatusData({ error: (error as Error).message });
    } finally {
  ...
  const chainAliasToCanonical = useMemo(() => {
    const map = {};
  ...
  const { mergedTotals: chainTotals, rawTotals: rawChainTotals } = useMemo(() => {
