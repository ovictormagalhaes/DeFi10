...
interface LendingDataItem {
  position?: {
    ...
    tokens?: Array<{
      ...
      balance?: number;
      logo?: string;
      financials?: { totalPrice?: number; balanceFormatted?: number };
      totalPrice?: number;
    }>;
    supplyRate?: number;
    borrowRate?: number;
  ...
  additionalData?: {
    ...
    projections?: Array<{
      type: string;
      projection: { oneDay?: number; oneWeek?: number; oneMonth?: number; oneYear?: number };
    }>;
  };
  additionalInfo?: { healthFactor?: number };
}
...
type SortBy = 'protocol' | 'healthFactor' | 'value';
type HealthFactorFilter = 'all' | 'risk' | 'warning' | 'healthy';

/**
 * Exemplo 1: Substituição Simples
 * Trocar LendingCards por LendingGroupedView
 */
export const ExemploSimples: React.FC<LendingDataProps> = ({ lendingData }) => {
  return (
    <div>
 ...
 */
export const ExemploComToggle: React.FC<LendingDataProps> = ({ lendingData }) => {
 ...
 */
export const ExemploComFiltros: React.FC<LendingDataProps> = ({ lendingData }) => {
  const [viewMode, setViewMode] = useState<ViewMode>('grouped');
  const [sortBy, setSortBy] = useState<SortBy>('protocol');
