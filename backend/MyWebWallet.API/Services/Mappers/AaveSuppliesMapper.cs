...
public class AaveSuppliesMapper : IWalletItemMapper<AaveGetUserSuppliesResponse>
{
    ...
    public async Task<List<WalletItem>> MapAsync(AaveGetUserSuppliesResponse response, ChainEnum chain)
    {
        ...
        foreach (var supply in response.Data.UserSupplies)
        {
            ...
            {
                // Parse values safely
                if (!decimal.TryParse(supply.Balance.Amount.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out var amountFormatted))
                    continue;

                if (!decimal.TryParse(supply.Balance.Usd, NumberStyles.Float, CultureInfo.InvariantCulture, out var totalPriceUsd))
                    continue;
                ...
                Console.WriteLine($"AaveSupplies: {supply.Currency.Symbol} amount={amountFormatted} totalUSD={totalPriceUsd} unitPrice={unitPrice} decimals={decimals} isCollateral={supply.IsCollateral} canBeCollateral={supply.CanBeCollateral}");
                ...
                // Preenche informações de colateral no nível do item
            ...
            {
                Console.WriteLine($"Error processing Aave supply for {supply.Currency?.Symbol}: {ex.Message}");
                // Continue processing other supplies
            }
        }
    ...
    private static decimal SafeDivide(decimal numerator, decimal denominator)
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine($"AaveSupplies: Capping extreme unit price {result} to {MAX_REASONABLE_PRICE}");
        ...
        {
            Console.WriteLine($"AaveSupplies: Division overflow {numerator} / {denominator}, returning 0");
            return 0;
        }
    }
