...
namespace MyWebWallet.API.Plugins.Protocols
{
    ...
    public class UniswapV3Plugin : IDeFiProtocolPlugin
    {
        ...
        {
            _logger.LogInformation("Initializing Uniswap V3 plugin");
        ...
        {
            return _chainConfig.GetEnabledChains()
                .Where(chain => SupportsChain(chain));
        }
        ...
        public async Task<ValidationResult> ValidateConfigurationAsync(ChainEnum chain, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (config == null)
                {
                    ...
                    result.Errors.Add($"No Uniswap V3 configuration found for chain {chain}");
                    return result;
                }
                ...
                {
                    result.IsValid = false;
                    result.Errors.Add($"Uniswap V3 is disabled for chain {chain}");
                    return result;
                }

                // Validate required contract addresses
                if (string.IsNullOrEmpty(config.Factory))
                {
                    result.Errors.Add($"Factory address is missing for chain {chain}");
                }

                if (string.IsNullOrEmpty(config.PositionManager))
                {
                    result.Errors.Add($"Position Manager address is missing for chain {chain}");
                }

                // Validate RPC connectivity
                var chainValidation = _chainConfig.ValidateChainConfig(chain);
                if (!chainValidation.IsValid)
                {
                    result.Errors.AddRange(chainValidation.Errors);
                }
            ...
            {
                _logger.LogError(ex, "Error validating Uniswap V3 configuration for chain {Chain}", chain);
                result.IsValid = false;
        ...
        public async Task<List<WalletItem>> GetWalletItemsAsync(string accountAddress, ChainEnum chain, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                // Convert Uniswap V3 positions to WalletItems using existing mapper
                var mapperLogger = _loggerFactory.CreateLogger<Services.Mappers.UniswapV3Mapper>();
                var mapper = new Services.Mappers.UniswapV3Mapper(
                    _uniswapV3Service, 
                    mapperLogger, 
                    new Aggregation.TokenFactory());
        ...
        public async Task<HealthCheckResult> CheckHealthAsync(ChainEnum? chain = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var chainsToCheck = chain.HasValue 
                    ? new[] { chain.Value }
                    : GetSupportedChains();
                ...
                {
                    var validation = await ValidateConfigurationAsync(chainToCheck, cancellationToken);
                ...
                if (allHealthy)
                {
                    return HealthCheckResult.Healthy(stopwatch.Elapsed, "All configurations valid");
                }
                else
                {
                    return HealthCheckResult.Unhealthy("Configuration validation failed", errors);
                }
            }
            catch (Exception ex)
            {
                stopwatch.Stop();
                _logger.LogError(ex, "Health check failed for Uniswap V3 plugin");
                return HealthCheckResult.Unhealthy("Health check exception", new[] { ex.Message });
            }
        }
    }
}